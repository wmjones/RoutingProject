...
# attn_cell = tf.contrib.seq2seq.AttentionWrapper(out_cell, attention_mechanism, attention_layer_size=hidden_dim/2)
out_cell = tf.contrib.rnn.OutputProjectionWrapper(out_cell, 2)
outputs = []
for i in range(steps_out):
    if i == 0:
        output, state = out_cell(tf.zeros((batch_size, 2)), encoder_final_state)

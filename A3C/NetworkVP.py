...
class NetworkVP:
    ...
    def summary(self, state, or_cost, or_route, depot_location, last_pred_route, pred_cost, sampled_cost):
        ...
        feed_dict = {self.raw_state: state, self.or_cost: or_cost, self.or_route: or_route,
                     self.start_tokens: depot_location, self.last_pred_route: last_pred_route,
                     self.sampled_cost: sampled_cost}
        _, _, summary, loss, diff, pred_route = self.sess.run([self.train_actor_op, self.train_critic_op, self.merged,

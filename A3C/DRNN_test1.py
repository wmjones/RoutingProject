...
cell = tf.contrib.rnn.OutputProjectionWrapper(cell, 2)
dec_outputs, dec_memory = tf.contrib.legacy_seq2seq.basic_rnn_seq2seq(enc_inp, dec_inp, cell)   # dtype=tf.float32)
# reshaped_outputs = [(tf.matmul(i, w_out)) for i in dec_outputs]  # + b_out

loss = 0
for _y, _y_hat in zip(dec_outputs, expected_sparse_output):
...
with tf.Session() as sess:
    ...
    feed_dict = {enc_inp[t]: x_batch[t] for t in range(seq_length)}
    # feed_dict.update({expected_sparse_output[t]: y_batch[t] for t in range(seq_length)})
    print(sess.run(dec_inp, feed_dict=feed_dict))

    feed_dict = {enc_inp[t]: x_batch[t] for t in range(seq_length)}
    outputs = np.array(sess.run([dec_outputs], feed_dict)[0])
...
for j in range(batch_size):
    ...
    for k in range(output_dim):
        ...
        plt.plot(range(len(past)-original_length+offset, len(expected)+len(past)-original_length+offset),
                 expected, "x--b", label=label2)
        plt.plot(range(len(past)-original_length+offset, len(pred)+len(past)-original_length+offset), pred, "o--y", label=label3)
